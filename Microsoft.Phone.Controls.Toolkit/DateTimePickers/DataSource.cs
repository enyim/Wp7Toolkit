...
namespace Microsoft.Phone.Controls
{
	abstract class DataSource : ILoopingSelectorDataSource
	{
		...
		{
			var next = GetRelativeTo(((DateTimeWrapper)relativeTo).DateTime, 1);

			return (next.HasValue)
					? ((LowerBound.HasValue && next < LowerBound) || (UpperBound.HasValue && next > UpperBound))
						? null
						: new DateTimeWrapper(next.Value)
					: null;
		}
